...
|---|---|---|---|
| record | ~~unified@240ms~~ → **nemotron@80ms** (see [Decision audit](#decision-audit--drift-from-original-decisions-2026-08-23)) | 240→80 ms | Nothing visible mid-recording; only final accuracy + how fast the last chunk resolves after stop. Unified's windowed decode runs ~15× realtime natively vs nemotron's ~2.5× → unusable post-stop waits; record now shares nemotron for legacy-parity latency (docs/PHASE11-NOTES.md). |
...
7. ✅ **Done** — Onboarding & model switching: `ModelSpec` registry ([src/asr/model.rs](../src/asr/model.rs)), `%APPDATA%/raycast-dictation/config.json` persistence ([src/config.rs](../src/config.rs), skip-onboarding once present, unknown ids fall back with warning), single-window content swap to an onboarding screen on first launch: RAM/core detection via sysinfo with min-RAM warning (never blocks), per-registry-model cached-state chips (`is_model_cached` scans the hf-hub cache dir offline), "use for record/live" assignment buttons, background download via `ensure_model_by_spec` streaming progress into the view over mpsc; F9 during onboarding queues through the existing pending-start flag and auto-starts once models land; worker now takes `ModelSelection {record, live}` instead of deriving kinds from mode alone.
